...
foreach($x in $i){
    ...
    # The validated direct index is authoritative for Daily Chunk text too. Offline sharing must
    # never overwrite a game-specific/franchise/genre decision with raw candidate-source text.
...
$m=[ordered]@{
    format='gamebrowser-daily-chunks-v2';schemaVersion=2;generatedAt=(Get-Date).ToUniversalTime().ToString('o')
    totalChunks=$compat.Count;directIndexGames=$kept.Count;seriesRules=$s.Count
    platformCounts=$counts;directIndexPlatformCounts=$ic
    note='OFFLINE repack only. Windows requires Steam AppID; IGDB-only rows require positive IGDB IDs; DAT-backed rows require IDs present in the local catalog. No network/API/build scans.'
}
...
try{
    ...
    foreach($fn in @('daily_chunks.json','daily_chunk_series.json','daily_chunk_index.json','manifest.json')){
...
Write-Host ''
Write-Host ("OFFLINE package ready: {0} chunks / {1} indexed games" -f $compat.Count,$kept.Count) -ForegroundColor Green
